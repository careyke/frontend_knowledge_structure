...
1. **时间切片** —— 告诉React应用当前帧中是否还有空余的时间，以便应用决定是否中断当前任务。
...
1. Reconciler将更新工作**由递归修改成了可以中断的循环过程**。每次循环开始之前都会调用 `shouldYield` 来判断是否需要中断任
...
2. **Reconciler和Renderer不再交替工作**，而且在Reconciler中对于变化的`fiber`节点打上标记，当所有的节点都完成Reconciler的工作时，才会统一交给Renderer来执行DOM操作更新视图。所以不会出现更新不完全的中间状态
